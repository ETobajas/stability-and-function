...
3.  The stability of plant's visitation rate is driven by the asynchrony of pollinator species abundances but not by the pollinator richness.
    [Likewise, we found a strong positive relationship between richness and asynchrony of pollinators.](Yo%20esto%20no%20lo%20meteria%20aquí,%20por%20que%20no%20aporta%20mucho,%20no?%20Y%20es%20complejo%20de%20explicar.)
    In contrast, the stability of the plant reproductive success is not related to the stability of the visitation rate.
    A year by year analysis revels that plants are unlikely to be limited by pollinator abundances except in specific years with [extreme low levels of pollinator activity](esto%20es%20así?%20Molaria).
...
[comment](aqui%20hecho%20a%20faltar%20la%20alternativa,%20solo%20la%20asyncronia%20entre%20las%20especies%20más%20abundantes%20es%20lo%20que%20cuenta)
...
[However, there are not that many studies linking explicitly pollinator visitation rates directly with plant reproductive outputs](de%20esto%20estas%20segura?).
While some studies report that sites with higher visitation rates show higher plant reproductive outputs (REFS), other studies do not find such relationship (Hegland and Totland 2012, Lundgren et al 2013; Kaiser‐Bunbury et al., 2017).
...
[gap](I%20think%20something%20about%20temporal%20variation%20and%20pollen%20limitation)

Here, we use a 5-year dataset collected across X Mediterranean woodland patches to study the role of pollinator richness and asynchrony as drivers of stability of its visitation rates to X plant speices.
Further, we evaluate whether more stable pollinator communities foster a more stabale plant reproductive success along years, measuring both the stability of fruit and seed set.
...
These sites are similar in the plant community composition and the most abundant plant species are known to be attractive to flower visitors and to depend on pollinators to maximize its reproduction (e.g. dominated by X, y and Z).
...
For each plant individual, we counted the total number of flowers observed in each survey and at the end of the season, we recorded the number of fruits developed and the number of seeds per fruit in a subsample of X fruits.
...
Pollinator richness is calculated as the number of different species observed, fruit set as the proportion of fruits developed, and the seed set as the mean?
number of seed per fruit for each plant individual sampled per site.
...
[The synchrony index for each plant species in the plot was calculated following Loreau and de Mazancourt (2008), as:](al%20final%20he%20puesto%20que%20utilizamos%20este%20solo%20porque%20al%20corregir%20el%20esfuerzo%20de%20muestreo%20este%20y%20el%20de%20Gross%20ya%20no%20estan%20correlacionados.%20He%20analizado%20el%20de%20Gross%20y%20no%20hay%20relacion%20con%20la%20estabilidad%20de%20las%20visitas.%20Muchos%20trabajos%20que%20solo%20utilizan%20un%20indice%20no%20dicen%20que%20hayan%20probado%20otros%20como%20es%20el%20trabajo%20de%20Lazaro,%20y%20he%20visto%20que%20muchos%20ponen%20la%20ecuacion,%20si%20no%20te%20parece%20bien%20asi%20vuelvo%20a%20poner%20los%20tres%20y%20explico%20lo%20de%20la%20correlacion)
...
To detect whether the effect of pollinator richness and visitation rate is different across the years, we use all X plant species sampled.
We analyzed the interactive effect between richness and year, and between visitation rate and year on fruit proportion using binomial generalized linear mixed model, and on the number of seeds employing linear mixed model.
...
There is a strong positive relationship between stability of visitation rates and asynchrony of pollinator species abundances with greater visitation rate stability when pollinator asynchrony increases (Estimate ± standard error: 2.903 ± 1.134) (Fig 1A), whereas stability of visitation rate was not influenced by the pollinator richness (-0.001 ± 0.063) (Fig 1B).
We also found a positive relationship between asynchrony and pollinator richness (0.020 ± 0.005) (Fig 1C).
However, note that despite one data point has a high leverage on this last analysis?
(*C.crispus*), when we removed this point the tendencie remain unchanged.
...
For the relationship with pollinator richness, *C. crispus*, *C. ladanifer* and *C. salviifolius* showed a negative relationships wile *H. halimifolium* and *L. pedunculata* showed positiverelationships, explaining the overall lack of relationship across all five plants (Supplementary material-Fig 1).
...
Stability of fruit set [llamale siempre igual, yo propongo fruit y seed number] varied from 2.62 to 120.66 (mean ± SE: 19.806 ± 3.615), whereas the stability of seed number ranged 1.28 to 55.86 (7.018 ± 1.441).
When we analyzed whether the stability of the pollinator's visitation rate affected the stability of the plant reproductive success, we did not find a clear relationship when analyzing the fruit set (Estimate: -1.547; std. error: 2.611) (Fig 2A).
However, contrary to expected, the stability of seed number did show a negative relationship with the stability of pollinator's visitation rate (Estimate: -1.887; std. error: 0.959) (Fig 2B).
...
When we analysed whether the effect of pollinator richness and visitation rate on fruit proportion is different across the years, we found that both variables showed a significant interaction with year [(richness:year p = \<0.0001; visitation rate:year p = \<0.0001)](tengo%20que%20poner%20Anova%20tipo%20III???%20y%20algun%20dato%20mas%20como%20df??).[Aqui me pones p-values?]
